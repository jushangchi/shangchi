       ...
       <table class="table table-bordered table-striped mb-none dataTable no-footer" id="datatable-editable" role="grid" aria-describedby="datatable-editable_info"> 
        <thead> 
         <tr role="row">
          ...
          <th class="sorting" tabindex="0" aria-controls="datatable-editable" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 325px;">轮播图片</th>
        ...
        <tbody> 
         ...
         <tr class="gradeA even" role="row"> 
          ...
          <td><img src="{{$row->img}}" style="width:135px;height:60px;"></td>  
          <td>{{$row->status==1?'启用':'禁用'}}</td> 
          <td>
            <form action="/slid/{{$row->id}}" method="post">
            {{csrf_field()}}
              {{method_field("DELETE")}}
                <button class="bk-margin-5 btn btn-danger btn-circle" type="submit"><i class="fa fa-times"></i></button>
              
            </form>
             <a href="/slid/{{$row->id}}/edit" class="bk-margin-5 btn btn-warning btn-circle"><i class="fa fa-pencil-square-o"></i></a>
            </td>  
            </form> 
          </td> 
         </tr>
